...
public class ExampleTest1 {
    ...
    */
    ...
    public void T03_verifyOnlyFourVideosReturned() {
        ...
        Assert.assertEquals("Video Size is not equal to 4", 4, CommonMethod.getVideoIdList(jp).size());
    }
 
    @Test
    public void T04_duplicateVideoVerification() {
        //Verify that there is no duplicate video
        Assert.assertTrue("Duplicate videos exist!", CommonMethod.findDuplicateVideos(CommonMethod.getVideoIdList(jp)));
    }
    ...
    private void printTitlePubDateDuration (JsonPath jp) {
        for(int i=0; i < CommonMethod.getVideoIdList(jp).size(); i++ ) {
