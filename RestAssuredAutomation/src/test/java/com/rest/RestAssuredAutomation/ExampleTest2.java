...
import com.rest.RestAssuredAutomation.util.CommonMethod;
import com.rest.RestAssuredAutomation.util.RestUtil;
...
public class ExampleTest2 {
    ...
    */
    ...
    public void T01_StatusCodeTest() {
    	...
    	CommonMethod.checkStatusIs200(res);
    }
 
    @Test
    public void T02_SearchTermTest() {
        //Verify the response contained the relevant search term (519218045)
        Assert.assertEquals("Id does not match!", "519218045", CommonMethod.getVideoIdList(jp).get(0).toString());
    }
 
    @Test
    public void T03_verifyExtraFourVideosReturned() {
        //Verify that extra 4 video entries were returned as related videos
    	Assert.assertEquals("Related video Size is not equal to 4", 4, CommonMethod.getRelatedVideoIdList(jp).size());
    }
 
    @Test
    public void T04_duplicateVideoVerification() {
        //Check duplicate videos exist?
    	Assert.assertTrue("Duplicate videos exist!", CommonMethod.findDuplicateVideos(getMergedVideoLists()));
    }
    ...
    private ArrayList getMergedVideoLists (){
        return CommonMethod.mergeLists(CommonMethod.getVideoIdList(jp), CommonMethod.getRelatedVideoIdList(jp));
    }
        ...
        for(int i=0; i <getMergedVideoLists().size(); i++ ) {
            ...
            if(jp.get("items.title[" + i + "]") != null) {
                ...
                if (jp.get("items.related.title[" + i + "][" + i + "]") != null) {
                    for (int j = 0; j < CommonMethod.getRelatedVideoIdList(jp).size(); j++) {
